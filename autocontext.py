...
weights = [3, 2, 1]  # TODO: Change this dummy.
scattered_labels = scatter_labels(blocks, label_count, loop_runs, weights)
...
for i in range(loop_runs):
    print col.Fore.GREEN + "- Running autocontext loop %d of %d -" % (i+1, loop_runs) + col.Fore.RESET

    # Insert the subset of the labels into the project.
    split_blocks = scattered_labels[i]
    proj.replace_labels(label_data_nr, split_blocks, block_slices)
